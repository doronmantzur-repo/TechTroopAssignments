...
function findIndex(numbers, num, start = 0, end = numbers.length - 1) {
  const index = start + Math.floor((end - start) / 2);
  if (numbers[index] === num) {
    return index;
  } else if (numbers[index] > num) {
    return findIndex(numbers, num, 0, index - 1);
  } else {
    return findIndex(numbers, num, index + 1, numbers.length - 1);
  }
